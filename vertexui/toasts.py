...
so it can report over a fullscreen app, a video player, a kiosk -
anything that owns the screen.
...
Two details that are easy to get wrong and fail silently:
  ...
  * UpdateLayeredWindow wants premultiplied alpha, or everything washes
    ...
    tray = toasts.Toasts(anchor_titles=["My App"])
    tray.start()
...
def _make_box(kind, label, colours):
    ...
    # Side rule, rounded and inset - the accent belongs to the row, so it
    # runs the row's height rather than the full box edge-to-edge.
    bar_w, inset = 4, 6
    cv2.rectangle(box, (0, inset + 2), (bar_w, ROW_H - inset - 3), col + (255,), -1)
    cv2.circle(box, (bar_w // 2, inset + 2), bar_w // 2, col + (255,), -1, cv2.LINE_AA)
    cv2.circle(box, (bar_w // 2, ROW_H - inset - 3), bar_w // 2, col + (255,), -1,
               cv2.LINE_AA)
    _icon(box, 30, ROW_H // 2, 9, kind, col)
    cv2.putText(box, label, (50, ROW_H // 2 + 5), cv2.FONT_HERSHEY_DUPLEX,
